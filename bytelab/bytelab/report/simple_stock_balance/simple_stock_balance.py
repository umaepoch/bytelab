...
def execute(filters=None):
    ...
    for rows in data:
        ...
        if loop_count == 1:
            ...
            if skip_row == 1 and rows[11] > 0 and (rows[10] is None
                    or rows[10] <= 0.0):
...
# ............item_count = item_count + 1
            ...
            if item_prev == item_work:
                ...
                if skip_row == 1 and (rows[9] is None or rows[9]
                        == 0.0):
                    summ_data.append([
                        rows[0],
                        rows[1],
    ...
    for no_warehouse_item in no_warehouse_items_list:
        if no_warehouse_item not in items_entry_list:
            ...
            no_whse_item_details = \
                fetch_no_warehouse_item_details(no_warehouse_item)
            ...
            manufacturer_part_no = \
                no_whse_item_details[0]['manufacturer_part_no']
            case = no_whse_item_details[0]['case']

            no_whse_item_details1 = \
                fetch_no_warehouse_items_entry(filters)
    ...
    # ............item_count = item_count + 1
                ...
                if item_prev == item_work:
                    ...
                    if skip_row == 1 and rows[11] > 0 and (rows[10]
                            is None or rows[10] <= 0.0):
                        sum_data.append([
                            rows[0],
                            rows[1],
                            rows[2],
        ...
        list1 = []
        for data1 in sum_data:
            if filters.get('test'):
                value1 = filters.get('test')
                if value1.capitalize() in data1 or value1.lower() in data1 or value1.upper() in data1 or value1.title() in data1:

                    # print("data1",data1);

                    summ_data.append(data1)
...
def fetch_no_warehouse_items_list():
    ...
    bin_items_list = \
        frappe.db.sql(""" select item_code, sum(actual_qty) as qty  from `tabBin` group by item_code """
                      , as_dict=1)
...
def fetch_no_warehouse_item_details(no_warehouse_item):
    details = \
        frappe.db.sql(""" select item_group,item_name,detail,manufacturer,manufacturer_part_no,`case` from `tabItem` where item_code=%s """
                      , no_warehouse_item, as_dict=1)
    return details
...
def fetch_no_warehouse_items_entry(filters):
    ...
    if conditions is not '':
        return frappe.db.sql(""" select item_code,item_name,item_group,detail,manufacturer,manufacturer_part_no,`case` from `tabItem` where item_code is not NULL%s"""
...
def get_conditions(filters):
    ...
    if filters.get('item_code'):
        conditions += 'and sle.item_code = %s' \
            % frappe.db.escape(filters.get('item_code'), percent=False)

    if filters.get('item_group'):
        conditions += ' and it.item_group = %s' \
            % frappe.db.escape(filters.get('item_group'), percent=False)

    if filters.get('warehouse'):
        conditions += ' and sle.warehouse = %s' \
            % frappe.db.escape(filters.get('warehouse'), percent=False)

    if filters.get('item_name'):
        conditions += 'and it.item_name =%s' \
            % frappe.db.escape(filters.get('item_name'), percent=False)

    if filters.get('cases'):
        conditions += ' and it.case =%s' \
            % frappe.db.escape(filters.get('cases'), percent=False)

    if filters.get('detail'):
        conditions += 'and it.detail = %s' \
            % frappe.db.escape(filters.get('detail'), percent=False)

    if filters.get('mfr'):
        conditions += 'and it.manufacturer = %s' \
            % frappe.db.escape(filters.get('mfr'), percent=False)

    if filters.get('mfr_pn'):
        conditions += ' and it.manufacturer_part_no = %s' \
            % frappe.db.escape(filters.get('mfr_pn'), percent=False)
...
def get_conditions3(filters):
    ...
    if filters.get('item_group'):
        conditions += ' and item_group =%s' \
            % frappe.db.escape(filters.get('item_group'), percent=False)

    if filters.get('item_name'):
        conditions += ' and item_name = %s' \
            % frappe.db.escape(filters.get('item_name'), percent=False)

    if filters.get('cases'):
        conditions += ' and case =%s' \
            % frappe.db.escape(filters.get('cases'), percent=False)

    if filters.get('detail'):
        conditions += ' and detail = %s' \
            % frappe.db.escape(filters.get('detail'), percent=False)
    ...
    if filters.get('mfr_pn'):
        conditions += ' and manufacturer_part_no = %s' \
            % frappe.db.escape(filters.get('mfr_pn'), percent=False)

    return conditions


def filtered_value(filters):
    value1 = filters.get('test')
    print ('value1', value1)
    return value1
...
def get_stock_ledger_entries(filters):
    ...
    print ('conditions', conditions)
    ledger_value = \
        frappe.db.sql("""select sle.item_code,sle.warehouse, sle.posting_date, actual_qty, sle.valuation_rate,
...
def get_stock_ledger_entries_wo_sl(filters):
    ...
    if value1!="None":
    	stock_value = \
        	frappe.db.sql("""select it.item_code, "" as warehouse, "" as posting_date, 0 as actual_qty, 0 as valuation_rate,
			"" as company, "" as voucher_type, 0 as qty_after_transaction, 0 as stock_value_difference from tabItem it
			where not exists (select 1 from `tabStock Ledger Entry` sle where sle.docstatus < 2 and sle.item_code = it.item_code) %s"""
                      % conditions, as_dict=1)
    else:
	    print("entered in new else");
	    stock_value = \
        	frappe.db.sql("""select it.item_code, "" as warehouse, "" as posting_date, 0 as actual_qty, 0 as valuation_rate,
			"" as company, "" as voucher_type, 0 as qty_after_transaction, 0 as stock_value_difference from tabItem it
			where not exists (select 1 from `tabStock Ledger Entry` sle where sle.docstatus < 2 and sle.item_code = it.item_code) or value1 in  it.item_code %s"""
                      % conditions, as_dict=1)
...
def get_item_details(filters):
    ...
    items = \
        frappe.db.sql("""select name, item_name, stock_uom, item_group, brand, description,
							 default_supplier, manufacturer, `case`, manufacturer_part_no, detail
		from tabItem {condition}""".format(condition=condition),
                      value, as_dict=1)
...
def validate_filters(filters):
    if not (filters.get('item_code') or filters.get('warehouse')):
        sle_count = \
            flt(frappe.db.sql("""select count(name) from `tabStock Ledger Entry`"""
                )[0][0])
        if sle_count > 500000:
...
def get_total_stock(item_code):
    ...
    item_stock = \
        flt(frappe.db.sql("""select sum(actual_qty)
			from `tabStock Ledger Entry`
			where item_code=%s""",
            item_code)[0][0])

    stock_recon = \
        flt(frappe.db.sql("""select sum(qty_after_transaction)
			from `tabStock Ledger Entry`
